...
def create_model(model_name, input_size, num_classes, device, patch_size=4, resume=None):
	...
	if model_name == "ResNet34":
		...
		# model = WideResNet(34, num_classes=num_classes)
		model = WideResNet(40, num_classes, 4, 0.3)
	elif model_name == "FCN":
		model = FCN()
		...
		model = ViT(image_size=input_size, patch_size=patch_size, num_classes=num_classes, dim=192, depth=12, heads=3, mlp_dim=768, dropout=0.1, emb_dropout=0.1)
	elif model_name == "ViT-S":
		model = ViT(image_size=input_size, patch_size=patch_size, num_classes=num_classes, dim=384, depth=12, heads=6, mlp_dim=1536, dropout=0.1, emb_dropout=0.1)
	elif model_name == "ViT-B":
		model = ViT(image_size=input_size, patch_size=patch_size, num_classes=num_classes, dim=768, depth=12, heads=12, mlp_dim=3072, dropout=0.1, emb_dropout=0.1)
